...
print(
    "Welcome to the Weather App! This app will allow you to search for the "
    "weather in any city in the world, and pollen data for selected cities."
)
        ...
        if choice == "1":
            ...
            while True:
                ...
                if country_code is None:
                    print(
                        "Invalid country name. Enter a valid country name."
                    )
                    continue
                ...
                url = (
                    ...
                    f"q={city_input},{country_code.alpha_2}&appid={api_key}"
                )
                response = requests.get(url)
                data = response.json()
                if response.status_code == 404 or not data:
                    print(
                        "City not found. Enter a valid city name and country."
                    )
                    continue
                lat = data[0]["lat"]
                lon = data[0]["lon"]
                ...
                # Extract the current weather data for that city
                    ...
                    sunrise = dt.datetime.fromtimestamp(
                        weather_data["sys"]["sunrise"] + timezone_offset
                    )
                    sunset = dt.datetime.fromtimestamp(
                        weather_data["sys"]["sunset"] + timezone_offset
                    )
                    ...
                    print("--------------------------------------------------")
                    print(
                        f"The temperature in {city_input} is: {temp}ºF or "
                        f"{temp_celsius}ºC\n"
                    )
                    print(
                        f"The temperature feels like: {feels_like_F}ºF or "
                        f"{feels_like_C}ºC\n"
                    )
                    print(
                        f"Wind speed in {city_input} is: {wind_speed} mph\n"
                    )
                    print(f"The humidiy in {city_input} is: {humidity}%\n")
                    print(
                        f"The description in {city_input} is: {description}\n"
                    )
                    print(f"The sunrise in {city_input} is: {sunrise}\n")
                    print(f"The sunset in {city_input} is: {sunset}\n")
                    print(
                        f"The current rainfall in {city_input} is: "
                        f"{rain.get('1h', 0)}mm\n"
                    )
                    print("--------------------------------------------------")
                    break
            ...
            while True:
                ...
                forecast_data = requests.get(
                    ...
                    f"q={city_input},{country_code}&appid={api_key}"
                ).json()
                ...
                # Extract the 5-day weather forecast data for that city
                    ...
                    for forecast in forecast_data["list"]:
                        forecast_time = dt.datetime.fromtimestamp(
                            forecast["dt"]
                        )
                        if forecast_time.date() != current_date:
                            ...
                            print(
                                f"General Weather: {description.capitalize()}"
                            )
                            print(
                                f"- Temperature: {temp_celsius:.2f}°C or "
                                f"{temp_fahrenheit:.2f}°F"
                            )
                            print(
                                f"- Feels Like: {feels_like_celsius:.2f}°C or "
                                f"{feels_like_fahrenheit:.2f}°F"
                            )
            ...
            while True:
                ...
                if not city_name:  # Check if the input is empty
                    print(
                        "Input cannot be empty.Enter a valid city name."
                    )
                    continue
                url = (
                    f"http://api.openweathermap.org/geo/1.0/direct?"
            ...
            pollen = "https://air-quality-api.open-meteo.com/v1/air-quality"
            params = {
                "latitude": lat,
                "longitude": lon,
                "current": [
            ...
            responses = openmeteo.weather_api(pollen, params=params)
            ...
            print("-------------------------------------------------------")
            print(
                f"Current alder pollen {current_alder_pollen} Grains/m³\n"
            )
            print(
                f"Current birch pollen {current_birch_pollen} Grains/m³\n"
            )
            print(
                f"Current grass pollen {current_grass_pollen} Grains/m³\n"
            )
            print(
                f"Current mugwort pollen {current_mugwort_pollen} Grains/m³\n"
            )
            print(
                f"Current olive pollen {current_olive_pollen} Grains/m³\n"
            )
            print(
                f"Current ragweed pollen {current_ragweed_pollen} Grains/m³\n"
            )
            print("-------------------------------------------------------")
        elif choice == "4":
            cache_session = requests_cache.CachedSession(
            ...
            two_hourly_data["Temperature °C"] = hourly_temperature_2m[
                ::2
            ]  # Select every 2nd hour
            two_hourly_data["Relative humidity %"] = (
                hourly_relative_humidity_2m[::2]
            )
            two_hourly_data["Rain mm"] = hourly_rain[::2]
            two_hourly_data["Snowfall"] = hourly_snowfall[::2]
            two_hourly_data["Wind speed kph"] = (
                hourly_wind_speed_10m[::2]
            )
